 ...
 * @since 1.0.0
 */
function toffedassen_entry_footer() {
	if ( ! has_tag() && ! intval( toffedassen_get_option( 'show_post_social_share' ) ) ) {
		return;
	}
 ...
 */
function toffedassen_content_limit( $num_words, $more = "&hellip;" ) {
 ...
 * @since  1.0
 */
function toffedassen_entry_thumbnail( $size = 'thumbnail' ) {
	$html = '';
 ...
 *
 */
function toffedassen_author_box() {
	if ( toffedassen_get_option( 'show_author_box' ) == 0 ) {
		return;
	}
 ...
 *
 */
function toffedassen_entry_meta() {
	$fields = (array) toffedassen_get_option( 'blog_entry_meta' );
 ...
 *
 */
function toffedassen_single_entry_meta() {
 ...
 */
function toffedassen_entry_meta_single_portfolio() {
	$time_string = '<time class="entry-date published updated" datetime="%1$s">%2$s</time>';
...
if ( ! function_exists( 'toffedassen_language_switcher' ) ) :
	function toffedassen_language_switcher( $show_name = false ) {
		$language_dd = '';
 ...
 */
if ( ! function_exists( 'toffedassen_currency_switcher' ) ) :
	function toffedassen_currency_switcher( $show_desc = false ) {
		$currency_dd = '';
...
function toffedassen_rating_stars( $score ) {
...
if ( ! function_exists( 'toffedassen_is_blog' ) ) :
	function toffedassen_is_blog() {
 ...
 * @return bool
 */
if ( ! function_exists( 'toffedassen_is_catalog' ) ) :
	function toffedassen_is_catalog() {
...
if ( ! function_exists( 'toffedassen_is_portfolio' ) ) :
	function toffedassen_is_portfolio() {
...
if ( ! function_exists( 'toffedassen_taxs_list' ) ) :
	function toffedassen_taxs_list( $taxonomy = 'category' ) {
...
if ( ! function_exists( 'toffedassen_blog_description' ) ) :
	function toffedassen_blog_description() {
		if ( ! toffedassen_is_blog() ) {
			return;
		}

		$blog_text = toffedassen_get_option( 'blog_page_header_subtitle' );
 ...
 * @return string
 */
function toffedassen_get_post_meta( $meta ) {
...
if ( ! function_exists( 'toffedassen_get_page_header' ) ) :
	function toffedassen_get_page_header() {
		...
		if ( toffedassen_get_post_meta( 'hide_page_header' ) ) {
			return false;
		}
		...
		if ( toffedassen_is_blog() ) {
			if ( ! intval( toffedassen_get_option( 'blog_page_header' ) ) ) {
				return false;
			}

			$pg_layout             = toffedassen_get_option( 'blog_page_header_layout' );
			$page_header['layout'] = $pg_layout;
			if ( in_array( $pg_layout, array( '2', '3' ) ) ) {
				$page_header['bg_image']   = toffedassen_get_option( 'blog_page_header_background' );
				$page_header['parallax']   = intval( toffedassen_get_option( 'blog_page_header_parallax' ) );
				$page_header['text_color'] = toffedassen_get_option( 'blog_page_header_text_color' );
			}

		} elseif ( is_page() ) {
			if ( ! intval( toffedassen_get_option( 'page_header' ) ) ) {
				return false;
			}

			$bg_image   = toffedassen_get_option( 'page_header_background' );
			$parallax   = intval( toffedassen_get_option( 'page_header_parallax' ) );
			$text_color = toffedassen_get_option( 'page_header_text_color' );
			...
			if ( ! intval( toffedassen_get_option( 'portfolio_page_header' ) ) ) {
				return false;
			}

			$page_header['bg_image']   = toffedassen_get_option( 'portfolio_page_header_background' );
			$page_header['parallax']   = intval( toffedassen_get_option( 'portfolio_page_header_parallax' ) );
			$page_header['text_color'] = toffedassen_get_option( 'portfolio_page_header_text_color' );
		}
...
if ( ! function_exists( 'toffedassen_get_breadcrumbs' ) ) :
	function toffedassen_get_breadcrumbs() {
		if ( toffedassen_is_blog() ) {
			if ( ! intval( toffedassen_get_option( 'blog_page_header_breadcrumbs' ) ) ) {
				return;
			}

		} elseif ( toffedassen_is_catalog() ) {
			if ( ! intval( toffedassen_get_option( 'catalog_page_header_breadcrumbs' ) ) ) {
				return;
			}

		} elseif ( toffedassen_is_portfolio() ) {
			if ( ! intval( toffedassen_get_option( 'portfolio_breadcrumb' ) ) ) {
				return;
			}

		} else {
			if ( ! intval( toffedassen_get_option( 'page_header_breadcrumbs' ) ) ) {
				return;
			}
 ...
 * @return string
 */
if ( ! function_exists( 'toffedassen_get_page_base_url' ) ) :
	function toffedassen_get_page_base_url() {
 ...
 * @return string
 */
if ( ! function_exists( 'toffedassen_get_product_video' ) ) :
	function toffedassen_get_product_video() {
		global $product;
 ...
 * @return string
 */
if ( ! function_exists( 'toffedassen_get_product_zoom' ) ) :
	function toffedassen_get_product_zoom() {
		if ( in_array( toffedassen_get_option( 'single_product_layout' ), array( '3', '4', '5', '6' ) ) ) {
			return;
		}

		if ( ! intval( toffedassen_get_option( 'product_images_lightbox' ) ) ) {
			return;
		}
